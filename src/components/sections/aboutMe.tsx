...
const AboutMe: React.FC<Props> = ({ isVisible, volume, openWindow}) => {
  return (
    ...
    >
      ...
      {/* --- WINDOW CONTENT --- */}
      <div className="p-4 flex-grow flex flex-col gap-y-3 overflow-y-auto transition-all duration-150 ease-in-out">
        <div className="text-center pb-3 border-b-2 border-[var(--border-color)] transition-all duration-150 ease-in-out">
          ...
          <p className="italic text-[var(--text-color-2)] transition-all duration-150 ease-in-out">The ultimate ghost of the web.</p>
        </div>
        ...
        <p className="text-[var(--text-color)]/70 transition-all duration-150 ease-in-out">If you have any further questions, feel free to ask me in <button onClick={() => openWindow('Comments')} className="underline text-[var(--text-color-2)] pointer-click transition-all duration-150 ease-in-out">comments</button></p>
      </div>
    </section>
  );
};
