...
const AboutMe: React.FC<Props> = ({ isVisible, volume, openWindow}) => {
  return (
    ...
    >
      ...
      {/* --- WINDOW CONTENT --- */}
      <div className="p-4 flex-grow flex flex-col gap-y-3 overflow-y-auto ">
        <div className="text-center pb-3 border-b-2 border-[var(--border-color)] ">
          ...
          <p className="italic text-[var(--text-color-2)] ">The ultimate ghost of the web.</p>
        </div>
        ...
        <p className="text-[var(--text-color)]/70 ">If you have any further questions, feel free to ask me in <button onClick={() => openWindow('Comments')} className="underline text-[var(--text-color-2)] pointer-click ">comments</button></p>
      </div>
    </section>
  );
};
