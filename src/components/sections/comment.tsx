...
const Comments: React.FC<Props> = ({ isVisible, volume }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.error("Spill the tea, Firebase:", error);
      },
    );
    ...
    return (
      <div className="" style={style}>
        ...
        >
          ...
          <div>
            <p className="font-extrabold text-[var(--text-color-2)]">
              {Comments.name}
              <span className="overflow-hidden rounded-2xl h-fit w-fit mx-2 ">
                {Comments.isPinned ? "✅" : ""}
              </span>
            </p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between mt-2 ">
            <div className="flex flex-col text-left ">
              <label
                htmlFor="name"
                className="text-sm font-bold text-[var(--text-color)]/70 "
              >
                Name (Optional)
              </label>
              <input
