...
const ProjectCard: React.FC<ProjectCardProps> = ({ imageUrl, webPath, title, description, className = '', name, setTop, id }) => {
  return (
    ...
    `}>
      ...
      <div className="p-3 text-center transition-all duration300 ease-in-out">
        <a href={webPath} title={name} target="_blank" className="text-xl decoration underline underline-offset-4 font-bold text-[var(--text-color-2)] transition-all duration-150 ease-in-out">{title}</a>
        <p className="text-sm text-[var(--text-color)] transition-all duration-150 ease-in-out">{description}</p>
      </div>
    </div>
  );
};
